<template>
  <div v-if="field">
    <div v-if="field.type === 'text' || field.type === 'number' || field.type === 'date' || field.type ==='email'">
        <input class="form-control"  :min="field.type === 'number' ? 0: false" :step="field.type === 'number' ? 'any' : false"  v-model="field.selected" :type="field.type" @blur="field.type === 'number' ? preventTwoDots(field.selected, field) : false"></input>
    </div>
    ...
    <b-form-select v-else-if="field.type === 'select'" v-model="field.selected" :options="field.options"></b-form-select>
