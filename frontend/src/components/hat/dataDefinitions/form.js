...
const getFormHat = ($gettext) => {
  const form = {
    formDetails: {
      tabsDisplay: ['sub_info', 'files', 'has_imports', 'has_produced'],
      dataNeeded: [
    ...
    tabs: {
      ...
      has_imports: {
        ...
        titleHtml: `<b>${$gettext('Consumption (imports)')}</b> <br><small>${$gettext('Annex F substances for exempted subsectors')} <br> ${$gettext('in metric tons (not ODP or CO2-equivalent tonnes)')}</small>`,
