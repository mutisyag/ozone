  ...
  <div v-if="tabName && parties">
		<b-row>
			...
			<b-col class="flex-center">
				<b-btn-group>
					<b-btn :disabled="!selected_countries.selected.length" @click="addEntries" variant="primary">
						<span v-translate="selected_countries.selected.length ? {length: selected_countries.selected.length} : {length: ''}">Add %{length} countries</span>
					</b-btn>
					<b-btn v-if="selected_countries.selected" @click="resetData"><span v-translate>Cancel</span></b-btn>
				</b-btn-group>
			</b-col>
		</b-row>
    </div>
